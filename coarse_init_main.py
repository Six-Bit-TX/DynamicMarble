...
def main(model, img_folder_path, n_views, device="cuda", batch_size=10, niter=300, schedule="linear", lr=0.01, focal_avg=True):
    ##########################################################################################################################################################################################

    train_img_list = sorted(os.listdir(img_folder_path))
    if not len(train_img_list)==n_views:
        print(f"Number of images ({len(train_img_list)}) in the folder ({img_folder_path}) is not equal to {n_views}")
        return False
    ...
    loss = compute_global_alignment(scene=scene, init="mst", niter=niter, schedule=schedule, lr=lr, focal_avg=focal_avg)
